...
namespace Iot.Device
{
    public class Max7219 : IDisposable
    {
        ...
        /// <summary>
        /// Contains the values of the digits Registers per device
        /// </summary>
        ...
        /// </summary>
        /// <value></value>
        public int Cascaded { get; private set; }

        /// <summary>
        /// The Rotation to be applied (when modules are assembled in a different way)
        /// </summary>
        ...
        /// </summary>
        public Max7219(SpiDevice spiDevice, int cascaded = 1)
        {
            _spiDevice = spiDevice;
            Cascaded = cascaded;
            _buffer = new byte[Cascaded, NumDigits];
            _writeBuffer = new byte[2 * Cascaded];
        }
        ...
        {
            var i = 0;
            for (byte deviceId = 0; deviceId < Cascaded; deviceId++)
            {
        ...
        {
            if (deviceId < 0 || deviceId >= Cascaded)
        ...
        private void ValidateBuffer(byte[,] buffer)
        {
            ...
            if (buffer.GetUpperBound(0) != Cascaded - 1)
                throw new ArgumentException(nameof(buffer), $"buffer upper bound ({buffer.GetUpperBound(0)}) for dimension 0 must be {Cascaded - 1}.");
        ...
        public void WriteBuffer(byte[,] buffer, RotationType rotation)
        {
            switch (rotation)
            {
                case RotationType.None: WriteBuffer(buffer); break;
        ...
        /// <summary>
        /// Writes the buffer to the devices.
        /// </summary>
        public void Flush()
        {
            WriteBuffer(_buffer, Rotation);
        }
